...
public class NUnitConfigurationProducer extends RunConfigurationProducer<NUnitConfiguration> {
    ...
    @Override
    protected boolean setupConfigurationFromContext(NUnitConfiguration configuration, ConfigurationContext context, SimpleReference<PsiElement> sourceElement) {
    ...
    private static Module getModuleForRun(ConfigurationContext configurationContext) {
        ...
        NUnitModuleExtension extension = module.getExtension(NUnitModuleExtension.class);
        if (extension != null) {
            return module;
        }
        return null;
